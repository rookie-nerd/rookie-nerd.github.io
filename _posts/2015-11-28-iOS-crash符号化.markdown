...
# crashLog相关介绍
正急需解crash的小伙伴可以直接跳到章节[crashlog解密实战](#crashPractice)。
...
0000000100000000 T __mh_execute_header
                 ...
                 U dyld_stub_binder
```
仔细观察两者的差别，可以发现strip之后，a.out文件变小了，而其中很多符号也被移除了，比如\_main,GCC_except_table2等。一般正式发布的二进制都会将将符号表strip，以便减少安装包的大小。
...
# <h2 id="crashPractice">crashlog解密实战</h2>
...
```asm
target create --no-dependents --arch armv7 AFWealth.app/AFWealth  // 这里arch需要根据crash log中的二进制文件部分查看
 
 # sample output
 # Current executable set to 'AFWealth.app/AFWealth' (armv7).

image list

 # sample output
 # [  0] 166C5B9E-A8CB-381B-9305-42BBAF3B979A 0x00004000 /Users/fragno/Desktop/crash/AFWealth.app/AFWealth 
 #     /Users/fragno/Desktop/crash/AFWealth.app.dSYM/Contents/Resources/DWARF/AFWealth

image load --file AFWealth __TEXT 0
 # sample output 
 # section '__TEXT' loaded at 0x0

image lookup -a 1184312
 # sample output
 # Address: AFWealth[0x00125238]  (AFWealth.__TEXT.__text + 1154248)
 #     Summary: AFWealth`-[AFWBaseModel codableProperties] + 212 at AFWBaseModel.m:151

image lookup -a 1184530
 # sample output
 # Address: AFWealth[0x00125312]  (AFWealth.__TEXT.__text + 1154466)
 # Summary: AFWealth`-[AFWBaseModel setWithCoder:] + 150 at AFWBaseModel.m:163
...
+ crash log（i.e. AFWealth.crash）,可以通过Xcode或者itunes导出[crash log的导出](https://developer.apple.com/library/ios/documentation/IDEs/Conceptual/AppDistributionGuide/AnalyzingCrashReports/AnalyzingCrashReports.html)
+ .app package（i.e. AFWealth.app），可以通过解压ipa包，从Payload中获得，也可以直接从机器上拷贝
    /User/Containers/Bundle/Application/9D8A82D6-DFE5-45FD-B994-77C93A294B03/AFWealth.app 真机上的app位置
+ .dSYM package，其包含调试信息（i.e. AFWealth.app.dSYM）
    /Users/fragno/Library/Developer/Xcode/DerivedData/AFWealth-eangdqkywlcbatcvllxataoasqrj/Build/Products 可在这个目录下找到

接着需要验证这三样东西是否匹配：

+ 查看crash log的UUID： Binary Images下的UUID
+ 使用dwarfdump查看binary package的UUID,[How to Match a Crash Report to a Build](https://developer.apple.com/library/ios/qa/qa1765/_index.html)
```shell
dwarfdump --uuid AFWealth.app/example
```

+ 查看.dSYM的UUID
```shell
dwarfdump --uuid AFWealth.app.dSYM
```
...
```shell
/Applications/Xcode.app/Contents/SharedFrameworks/DTDeviceKitBase.framework/Versions/A/Resources/symbolicatecrash -v AFWealth.crash 2> symbolicatecrash.log
...
```shell
xcrun atos -o AFWealth.app.dSYM/Contents/Resources/DWARF/AFWealth -l 0x29000 -arch armv7        // 这些参数都可以在crashlog文件的二进制文件部分获得
...
0x0014a238
 # sample output
 # -[AFWBaseModel codableProperties]  (in AFWealth) (AFWBaseModel.m:151)
0x0014a312
 # sample output
 # -[AFWBaseModel setWithCoder:]  (in AFWealth) (AFWBaseModel.m:163)
```
