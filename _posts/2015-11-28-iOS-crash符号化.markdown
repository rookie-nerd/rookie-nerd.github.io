...
学过C++的同学都知道，从编写源代码到生成可执行文件，中间需要经过以下几步，并辅以简单的hello world来说明。相关知识，[符号表](https://en.wikipedia.org/wiki/Symbol_table)
源程序     ->              编译           ->                 链接                 -> 可执行文件
hello.cpp                              hello.o                                      hello
(ASCII c program text)                 (Mach-O 64-bit object x86\_64)              (Mach-O 64-bit executable x86\_64)

编译生成的hello.o是一个object文件，**每个object文件都有自己的一张符号表，里面包含了所有外部可见的变量、函数等的标识符**，这样在链接的时候，链接器就会根据符号表去解析未解析的符号，否则就会链接失败，报undefined symbol错误。
+ 符号表可能仅仅存在编译链接过程，完成之后就被丢弃了（大部分发布的二进制文件都是不包含符号表的），
...
好了，以上就是解密crash的全部原理了，那么说了那么多，我们拿到crashlog之后，怎么样才能解出我们想要的log呢？下面部分就简单的介绍下实战方案。
